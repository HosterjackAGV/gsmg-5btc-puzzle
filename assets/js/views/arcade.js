...
export default async function arcadeView({ params, navigate }) {
  ...
  if (entry) return playView(entry, navigate);
  return hubView(navigate);
}

// ---------- play one game ----------
function playView(entry, navigate) {
  const html = `
  <section class="section"><div class="wrap">
    <div class="arc-playbar">
      ...
      <div class="arc-title"><span class="ico">${entry.icon}</span><b>${esc(entry.title)}</b>
        <span class="badge ${entry.kind}">${entry.kind === 'skill' ? 'ranked' : 'idle'}</span></div>
      <div class="faint sm">${entry.verifiable ? 'verified board' : 'XP & milestones'}</div>
    </div>
  ...
  async function mount(root) {
    ...
    try {
      ...
      stage.innerHTML = '';
      const host = hostMod.createHost({ el: stage, game: entry, navigate });
  ...
  return { title: entry.title, html, mount };
}
