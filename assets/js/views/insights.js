...
const CHAIN = [
  ...
  { t: `<b>The 2023 "reverse-binary" hint is the Rosetta Stone, and its method is the message.</b> Recovered byte-exact from jrk's post (161 = 7×23 bytes), it names the four ingredients and its decode is an <i>involution</i> — a self-inverse end-to-end mirror, the same shape as Beaufort (encrypt = decrypt) and the yin-yang's 180° symmetry — so the final <b>combine is self-inverse/symmetric, not exotic</b>, and the hint quietly performs the combine it "withholds" (zero-separator concatenation, then a mirror). Its token lengths even sort the tokens: the three ×13 lengths flag the KNOWN tokens, the rest are prime-powers with bases <b>{2,3,5,7}</b> — "the prime part" — and "promised" is an anagram of PRIMES+DO. Yet every value these readings imply sweeps to noise, which pins the bottleneck on the <code>yinyang</code> <i>value</i>, not the combine.`,
    refs: ['reverse-binary-hint-involution-prime-length-structure'] },
...
const CONCLUSION = [
  ...
  `Compute <code>yellowblueprimes</code> from the genesis along the creator's own emphasis — "the prime part" and <code>fefefe = 101 010</code> — i.e. the yellow &amp; blue cell numbers filtered by primes, with "zero out" / "fefefe" as the indexing rule (note: neither the small set {2,3,5,7} nor the grid-native A007522 set is <i>creator-confirmed</i>; both are our readings) — and validate each candidate by whether <code>sha256(candidate)</code> opens a small blob. (The genesis-<i>numeric</i> candidates reachable from the coloured-cell + primes rule, woven through the full 24-order recipe, are now themselves swept — null — so the missing value lies <i>outside</i> the obvious index/count readings.)`,
